...
Using other feature groups such as the `norm_cenrot` columns did yield good results, but it required a lot of features at 29 (possible overfitting?). 
...
After generating the pairwise euclidian distances, it became really obvious that we would need feature selection given the amount of features. In fact, if you tried to run this model with all these new features, you run the risk of way overfitting the model. We thought that forward feature selection would be a good choice, as we can easily identify how and which additional distances will improve the model. 
...
We saw that adding more features did yield improvements until 4 around features.
